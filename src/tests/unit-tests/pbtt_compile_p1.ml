...
let () =
  let s =
    "message TestM { enum TestE {TestE_Value1 = 1; TestE_Value2 = 2; }required \
     TestE teste_field = 1; }"
  in
  let m = parse Pb_parsing_parser.message_ s in
  let all_types =
    Pb_typing_validation.validate_message file_name file_options
      Tt_util.empty_scope m
  in
  assert (2 = List.length all_types);
  match List.nth all_types 0 with
  | { Tt.scope; spec = Tt.Enum { Tt.enum_name; Tt.enum_values; _ }; _ } ->
...
let () =
  ...
  let s = "message Test {required int64 ival  = 1;required string sval = 2;}" in
  let ast = parse Pb_parsing_parser.message_ s in
  let all_messages =
    Pb_typing_validation.validate_message file_name file_options
      Tt_util.empty_scope ast
  in
  assert (List.length all_messages = 1);
  (match List.hd all_messages with
  | { Tt.scope; spec = Tt.Message { Tt.message_name; Tt.message_body; _ }; _ }
    ->
  ...
  let s =
    "message Test {message Inner {required int64 ival  = 1;required string \
     sval = 2;}}"
  in
  let ast = parse Pb_parsing_parser.message_ s in
  let all_messages =
  ...
  | { Tt.scope; spec = Tt.Message { Tt.message_name; Tt.message_body; _ }; _ }
    ...
    assert (2 = List.length message_body);
    test_fields message_body;
    let expected_scope = { Tt.packages = []; Tt.message_names = [ "Test" ] } in
...
let () =
  let s = "message Test {required Msg1.Msg2.SubMessage mval  = 1;}" in
  let ast = parse Pb_parsing_parser.message_ s in
  let all_messages =
    Pb_typing_validation.validate_message "a.proto" file_options
      Tt_util.empty_scope ast
...
let () = print_endline "Pbtt Compile P1 ... Ok"
