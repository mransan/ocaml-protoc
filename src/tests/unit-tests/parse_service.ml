...
let expect asserted expected =
  if asserted = expected then
    ()
  else
    failwith (Printf.sprintf "Expected %s, but got %s" expected asserted)
...
let test_services () =
  ...
  in
  let () =
    match rpc_get_with_semicolon with
  ...
  let () =
    ...
    | Service_rpc { rpc_name; rpc_options; rpc_req; rpc_res; _ } ->
      ...
      let req =
        match rpc_req with
        | `User_defined r -> r
        | _ -> failwith "Unexpected rpc req type"
      in
      let res =
        match rpc_res with
        | `User_defined r -> r
        | _ -> failwith "Unexpected rpc res type"
      in
  ...
  in
  let () =
    match rpc_list_client_streaming with
    | Service_option _ -> failwith "Expected rpc, but got option"
    | Service_rpc
        {
          rpc_name;
  ...
  in
  let () =
    match rpc_list_server_streaming with
    | Service_option _ -> failwith "Expected rpc, but got option"
    | Service_rpc
        {
          rpc_name;
  ...
  in
  let () =
    match rpc_list_bidi_streaming with
    | Service_option _ -> failwith "Expected rpc, but got option"
    | Service_rpc
        {
          rpc_name;
