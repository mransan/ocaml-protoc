...
let () =
  let proto_file =
    "package foo.bar;message M1 { message M11 {} message M12 {} } message M2 { \
     message M21 {} message M22 {} } "
  in

  let proto =
    Pb_parsing_parser.proto_ Pb_parsing_lexer.lexer
      (Lexing.from_string proto_file)
  in
