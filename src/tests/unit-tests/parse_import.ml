...
let () =
  let proto =
    parse Pb_parsing_parser.proto_
      "import \"blah.proto\";import public \"boom.proto\";"
  in
  let imports = proto.Pt.imports in
  assert (
    [ Pt_util.import "blah.proto"; Pt_util.import ~public:() "boom.proto" ]
    = imports)

let () = print_endline "Parse Import ... Ok"
