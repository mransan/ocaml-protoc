...
let () =
  let s =
    "message Outer {required int64 ival = 1;required string sval = 2;message \
     Inner { required int64 inner_ival = 1;required string inner_sval = 2;} ; \
     /* make sure that the ';' is supported. */required Inner inner = 3; }"
  in
  (*
  Printf.printf "---- MESSAGE ----\n";
  *)
  let { Pt.message_name; Pt.message_body; _ } =
    parse Pb_parsing_parser.message_ s
  in
...
let () =
  let s =
    "message TestM { enum TestE {TestE_Value1 = 1; TestE_Value2 = 0x2; \
  ...
  *)
  ...
  | Pt.Message_enum { Pt.enum_name; Pt.enum_body; Pt.enum_id = _ } ->
...
let () =
  let s = " message M1 {} message M2 {}" in
  let proto = parse Pb_parsing_parser.proto_ s in
  let messages = proto.Pt.messages in
...
let () =
  let s = " package my.proto;message M1 {} message M2 {}" in
  let proto = parse Pb_parsing_parser.proto_ s in
...
let () =
  let s =
    " syntax = \"proto2\";import \"blah.proto\";import public \
     \"boom.proto\";package my.proto;message M1 {} message M2 {}extend  M2 {} "
  in
  let proto = parse Pb_parsing_parser.proto_ s in
...
let () =
  let s =
    "message M {required int32 ival = 1;message M2 {}  message M3 {} message \
     M4 {}"
  in
...
let () = print_endline "Parse Message ... Ok"
let () = print_endline "Parse Message ... Ok"
