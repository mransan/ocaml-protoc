...
let () =
  let do_test s =
    let { Pt.field_name; field_type; field_label; field_options; field_number }
        =
      parse Pb_parsing_parser.normal_field_ s
    in
    assert (field_name = "x");
...
let () =
  let { Pt.field_name; field_type; field_label; field_options; field_number } =
    parse Pb_parsing_parser.normal_field_ "optional .M1 x = 1;"
  in
  assert (field_name = "x");
  assert (field_label = `Optional);
...
let () =
  let do_test s =
    let { Pt.oneof_name; Pt.oneof_fields } = parse Pb_parsing_parser.oneof_ s in
    assert (oneof_name = "foo");
    assert (List.length oneof_fields = 2);
    let { Pt.field_name; Pt.field_number; Pt.field_type; Pt.field_options; _ } =
      List.nth oneof_fields 0
    in
  ...
  do_test "oneof foo {string name = 4;SubMessage sub_message = 9 [a=1];}";
...
let () = print_endline "Parse Fields ... Ok"
