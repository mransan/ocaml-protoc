...
let decode_pb_ref_data () =
  T.make_test ~double_value:(Some 1.23)
    ~float_value:(Some (round32 1.23))
    ~int32_value:(Some (-123l)) ~uint32_value:(Some 123l)
    ~int64_value:(Some (-123L)) ~uint64_value:(Some 123L)
    ~bool_value:(Some true) ~string_value:(Some "abc")
