...
let rec default_test_type () = Encode (0)
...
let rec encode_test_type v encoder = 
  match v with
  | Encode x -> (
...
let rec pp_test_type fmt = function 
...
let rec default_test_id () = Int_list

let rec decode_test_id d = 
  match Pbrt.Decoder.int_as_varint d with
  | 1 -> Int_list
  | 2 -> Int_repeated
  | 3 -> Int_packed_repeated
  | _ -> failwith "Unknown value for enum test_id"

let rec encode_test_id v encoder =
  match v with
...
let rec pp_test_id fmt = function
...
let rec encode_test_request v encoder = 
...
let rec encode_test_requests v encoder = 
  List.iter (fun x -> 
    Pbrt.Encoder.key (1, Pbrt.Bytes) encoder; 
...
let rec pp_test_requests fmt v = 
  let pp_i fmt () =
    F.pp_open_vbox fmt 1;
...
let rec encode_encode_data v encoder = 
...
let rec encode_decode_data v encoder = 
  Pbrt.Encoder.key (1, Pbrt.Bits64) encoder; 
...
let rec encode_test_response v encoder = 
  Pbrt.Encoder.key (1, Pbrt.Varint) encoder; 
...
let rec encode_test_responses v encoder = 
  List.iter (fun x -> 
    Pbrt.Encoder.key (1, Pbrt.Bytes) encoder; 
...
let rec pp_test_responses fmt v = 
  let pp_i fmt () =
    F.pp_open_vbox fmt 1;
...
let rec encode_int_list v encoder = 
  List.iter (fun x -> 
    Pbrt.Encoder.key (1, Pbrt.Varint) encoder; 
    Pbrt.Encoder.int_as_varint x encoder;
  ) v.int_list;
...
let rec encode_int_repeated v encoder = 
...
let rec encode_int_packed_repeated v encoder = 
  Pbrt.Encoder.key (1, Pbrt.Bytes) encoder; 
...
let rec pp_int_packed_repeated fmt v = 
  let pp_i fmt () =
    F.pp_open_vbox fmt 1;
