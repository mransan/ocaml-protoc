...
(** This module implements the execution of a test request for a given type.

    The type for which the test is run is specified as a module signature to be
    implemented by the testing application. *)
...
module type T_sig = sig
  ...
  val make : int -> t * int
  (** [make difficulty] creates a value of type [t] with corresponding
      difficulty. *)
...
module type Runner_sig = sig
  ...
  (** [run request] execute the corresponding request and returns the test
      response *)
end
