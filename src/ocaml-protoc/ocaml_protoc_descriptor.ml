...
let fq_name all_types id =
  ...
  in
  "." ^ String.concat "." (t.Tt.scope.Tt.packages @ t.Tt.scope.Tt.message_names @ [name])

let field_type_of_builtin : Pb_field_type.builtin_type -> D.field_descriptor_proto_type =
  function
...
let label_of_pt : Pt.message_field_label -> D.field_descriptor_proto_label = function
...
let field_of_message_field ~all_types ~oneof_index
  ...
  D.make_field_descriptor_proto
    ~name:f.Tt.field_parsed.Pt.field_name
...
let message_descriptor ~all_types ~scope (msg : Pb_field_type.resolved Tt.message) =
  let fields = ref [] in
  let nested = ref [] in
  let oneofs = ref [] in
  ...
  List.iter
    (fun body_item ->
      ...
      | Tt.Message_field f ->
        fields := !fields @ [ field_of_message_field ~all_types ~oneof_index:None f ]
      | Tt.Message_oneof_field oneof ->
        oneofs :=
          !oneofs @ [ D.make_oneof_descriptor_proto ~name:oneof.Tt.oneof_name () ];
      ...
      | Tt.Message_map_field map ->
        ...
        let key_type =
          field_type_of_builtin (map.Tt.map_key_type :> Pb_field_type.builtin_type)
        in
        let val_type, val_type_name = resolve_field_type ~all_types map.Tt.map_value_type in
        let entry =
          D.make_descriptor_proto
            ~name:entry_name
            ~field:
              [
                ...
                D.make_field_descriptor_proto ~name:"value" ~number:2l
                  ~label:D.Label_optional ~type_:val_type ?type_name:val_type_name ();
              ]
      ...
      (List.map
         (fun (rpc : Pb_field_type.resolved Tt.rpc) ->
           D.make_method_descriptor_proto ~name:rpc.Tt.rpc_name
             ...
             ~client_streaming:rpc.Tt.rpc_req_stream
             ~server_streaming:rpc.Tt.rpc_res_stream ())
         svc.Tt.service_body)
    ()
...
let to_file_descriptor_proto
  ...
  let message_type, enum_type =
    List.fold_right
      ...
      flat_types ([], [])
  in
  D.make_file_descriptor_proto ~name:(Filename.basename proto_file_name) ~package
    ~syntax:"proto3" ~message_type ~enum_type
    ~service:(List.map (service_descriptor ~all_types) typed_proto.Tt.proto_services)
    ()
...
let write_json ~out_file
    ...
    ~proto_file_name
    ~(typed_proto : Pb_field_type.resolved Tt.proto) =
  let file_desc = to_file_descriptor_proto ~all_types ~proto_file_name ~typed_proto in
